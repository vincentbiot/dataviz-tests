// Visualizations class adapted for htmlwidgets
class Visualizations {
    ...
    initTooltip() {
        ...
        d3.select(this.el).selectAll('.distroviz-tooltip').remove();

        this.tooltip = d3.select(this.el)
            .append('div')
            .attr('class', 'distroviz-tooltip')
    ...
    positionTooltip(event) {
        ...
        if (left + tooltipRect.width > this.width - padding) {
            left = event.clientX - containerRect.left - tooltipRect.width - padding;
        }

        if (top < padding) {
            top = padding;
        } else if (top + tooltipRect.height > this.height - padding) {
            top = this.height - tooltipRect.height - padding;
        }
    ...
    // Generate tooltip content for stats
    getStatsTooltipContent(stats, sampleName) {
        if (!stats) return '';

        return `
            <div style="font-weight: 600; margin-bottom: 4px;">${sampleName}</div>
            <div style="display: flex; justify-content: space-between; gap: 16px;">
                <span>Moyenne</span>
                <span style="font-weight: 600;">${stats.mean}</span>
            </div>
            <div style="display: flex; justify-content: space-between; gap: 16px;">
                <span>Médiane</span>
                <span style="font-weight: 600;">${stats.median}</span>
            </div>
            <div style="display: flex; justify-content: space-between; gap: 16px;">
                <span>Q1 (25%)</span>
                <span style="font-weight: 600;">${stats.q1}</span>
            </div>
            <div style="display: flex; justify-content: space-between; gap: 16px;">
                <span>Q3 (75%)</span>
                <span style="font-weight: 600;">${stats.q3}</span>
            </div>
        `;
    }
    ...
    getPointTooltipContent(index, value, sampleName) {
        return `
            <div style="font-weight: 600; margin-bottom: 4px;">${sampleName} - Point ${index + 1}</div>
            <div style="display: flex; justify-content: space-between; gap: 16px;">
                <span>Valeur</span>
    ...
    // Calculate common Y scale for all samples
    ...
    // Calculate outlier bounds using Tukey's fences
    ...
    // Draw establishment reference line
    drawEstablishmentLine(g, y, establishment) {
        ...
        if (establishment.value < yMin || establishment.value > yMax) {
            console.warn(`Establishment value ${establishment.value} is outside range [${yMin}, ${yMax}]`);
            return;
        }
        ...
        // Small circular marker on Y axis
        g.append('circle')
    ...
    drawBoxPlot(samples, showDataPoints, isComparison, establishment = null, showOutliers = true) {
        ...
        // Data points area on the left (if enabled)
        if (showDataPoints) {
        ...
        samples.forEach((sample, idx) => {
            ...
            // Outliers (if data points disabled and showOutliers enabled)
    ...
    drawDataPoints(g, data, y, leftBoundary, rightBoundary, outlierIndices, color, sampleName, showOutliers = true) {
        ...
        // Filter outliers if showOutliers is false
        if (!showOutliers) {
        ...
        g.selectAll(`circle.datapoint-${sampleName.replace(/\s+/g, '')}`)
            ...
            .attr('class', d => {
                ...
                return `datapoint datapoint-${sampleName.replace(/\s+/g, '')}${isOutlier ? ' outlier-point' : ''}`;
            })
